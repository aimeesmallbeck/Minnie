...
| `aimeecloud/device/{device_id}/in` | Robot → Cloud | Intents, game moves, disconnect, ping |
| `aimeecloud/device/{device_id}/out` | Cloud → Robot | **Legacy** broadcast: all responses, TTS text, game state updates, snapshot requests |
| `aimeecloud/device/{device_id}/session/{session_id}/out` | Cloud → Robot | **Preferred** session-scoped responses (avoids cross-talk when multiple processes are present) |
...
> **Important:** The robot must subscribe to `out` and `status` **before** publishing `connect`. If the `session_init` response contains a `response_topic`, the robot **must** subscribe to that session-specific topic and should unsubscribe from the device-wide `out` topic to avoid receiving responses intended for other processes.
...
```json
{
  ...
  "device_id": "arduino-uno-q-001",
  "status": "connected",
  "response_topic": "aimeecloud/device/arduino-uno-q-001/session/sess_abc123def4567890/out",
  "expires_in": 600,
  "ttl": 600,
  "timestamp": "2026-04-14T09:46:05Z"
}
...
**Robot action:**
- Store `session_id` in non-volatile or durable memory. All subsequent messages must include it.
- Subscribe to `response_topic` if provided. This is the only topic guaranteed to deliver responses for this session.
- Send `request_session_id` on every reconnect to resume the session and preserve context/game state.
...
When the robot powers down or goes to sleep, publish on the **`in`** topic:
...
> **Do not send `disconnect` to the `connect` topic.** The cloud treats messages on `connect` as session requests; a `disconnect` sent there will create an unintended new session.
>
...
### 3.6 Single Active Session per Device

The cloud enforces **one connected session per `device_id`**. When a new `connect` request arrives, any previous connected session for the same device is marked `disconnected`.

To avoid two robot processes hearing each other's responses:

1. Ensure only one voice / response process is active per `device_id` at a time.
2. Use the `response_topic` returned in `session_init` instead of the device-wide `out` topic.
3. If a process receives `sub_type: "session_invalid"`, it must stop sending messages and reconnect with the latest stored `session_id` (or without one to start fresh).

### 3.7 Heartbeat / Keep-Alive

The robot should send a `ping` at least every 30 seconds on the `in` topic:
...
The cloud responds with `sub_type: "pong"`. If the session is no longer active, the cloud responds with `sub_type: "session_invalid"` and the robot must reconnect.
...
Useful for keepalive or latency checks. Send at least every 30 seconds while connected:
...
**Cloud response on the active `response_topic` (or legacy `out`):**
...
{
  "type": "response",
  "sub_type": "session_invalid",
  "device_id": "arduino-uno-q-001",
  "session_id": "sess_abc123def4567890",
  "text": "Session is not active; please reconnect.",
  "timestamp": "2026-04-14T09:46:05Z"
}
```
...
mqtt.subscribe(f"aimeecloud/device/{device_id}/out")          # legacy fallback
...
def on_message(topic, payload):
    ...
    if data["type"] == "session_init":
        ...
        if response_topic:
            ...
            mqtt.unsubscribe(f"aimeecloud/device/{device_id}/out")

    elif data["type"] == "response":
        if data.get("sub_type") == "session_invalid":
            clear_session_id()
            reconnect()
            return
    ...
    elif data["type"] == "status" and data["status"] in ("expired", "disconnected"):
        clear_session_id()
```
...
### v1.5 Changes
- Enforced single active session per `device_id`.
- Added `response_topic` to `session_init` for session-scoped responses.
- Added `sub_type: "session_invalid"` response for inactive sessions.
- Clarified that `disconnect` and `ping` belong on the `in` topic.
- Added keep-alive recommendation (ping every 30 seconds).
